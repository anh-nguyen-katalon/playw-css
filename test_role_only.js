...
// Create custom engines that always return empty array except for internal:role
function createNullEngine() {
  return {
    queryAll: () => []
  };
}
// Init temporary injectedScript to get all engines
let tempInjectedScript = new window.katalonpw.InjectedScript(window, false, "javascript", "testId", 1, "chrome", []);
const engines = tempInjectedScript._engines;

const customEngines = [];
for (const [engineName, engine] of engines) {
  if (engineName !== "internal:role") {
    customEngines.push({
      "name": engineName,
      "engine": createNullEngine()
    })
  }
}
...
let injectedScript = new window.katalonpw.InjectedScript(window, false, "javascript", "testId", 1, "chrome", customEngines);
